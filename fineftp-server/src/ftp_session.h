...
namespace fineftp
{
  ...
  private:
    ...
    void sendDirectoryListing   (const std::map<std::string, std::shared_ptr<Filesystem::FileInfo>> directory_content);
    void sendNameList           (const std::map<std::string, std::shared_ptr<Filesystem::FileInfo>> &directory_content);

    void sendFile               (std::shared_ptr<Filesystem::FileIOInterface> file);

    void readDataFromFileAndSend(std::shared_ptr<Filesystem::FileIOInterface> file
  ...
  ////////////////////////////////////////////////////////
  private:
    void receiveFile(std::shared_ptr<Filesystem::FileIOInterface> file);

    void receiveDataFromSocketAndWriteToFile(std::shared_ptr<Filesystem::FileIOInterface> file
                                           , std::shared_ptr<asio::ip::tcp::socket> data_socket);

    void writeDataToFile(std::shared_ptr<std::vector<char>> data
                       , std::shared_ptr<Filesystem::FileIOInterface> file
                       , std::function<void(void)>          fetch_more = []() { return; });

    void endDataReceiving(std::shared_ptr<Filesystem::FileIOInterface> file);
