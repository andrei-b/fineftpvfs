...
namespace fineftp
{
  ...
  void FtpSession::handleFtpCommandRETR(const std::string& param)
  {
    ...
    std::shared_ptr<Filesystem::FileIOInterface> file = Filesystem::createFileInterfaceObject();

    if (!file->open(local_path))
    {
  ...
  void FtpSession::handleFtpCommandSTOR(const std::string& param)
  {
    ...
    auto existing_file_filestatus = Filesystem::createFileInfoObject(local_path);
  ...
  void FtpSession::handleFtpCommandRNTO(const std::string& param)
  {
    ...
    if (is_renamable_error.replyCode() == FtpReplyCode::COMMAND_OK)
    {
      ...
      if (Filesystem::createFileInfoObject(local_to_path)->isOk())
      {
  ...
  void FtpSession::handleFtpCommandDELE(const std::string& param)
  {
    ...
    auto file_status = Filesystem::createFileInfoObject(local_path);
  ...
  void FtpSession::handleFtpCommandLIST(const std::string& param)
  {
    ...
    auto dir_status = Filesystem::createFileInfoObject(local_path);
    ...
    if (dir_status->isOk())
    {
      if (dir_status->type() == Filesystem::FileType::Dir)
      {
        if (dir_status->canOpenDir())
        {
          ...
          sendDirectoryListing(dir_status->dirContent());
          return;
        }
        else
        {
    ...
    if (dir_status->isOk())
    {
      if (dir_status->type() == Filesystem::FileType::Dir)
      {
        if (dir_status->canOpenDir())
        {
          ...
          sendNameList(dir_status->dirContent());
          return;
        }
        else
        {
  ...
  void FtpSession::sendDirectoryListing(
          const std::map<std::string, std::shared_ptr<Filesystem::FileInfo>> directory_content)
  {
    ...
    data_acceptor_.async_accept(*data_socket
      ...
      // TODO: close acceptor after connect?
                                  ...
                                  for (const auto& entry : directory_content)
                                  {
                                    ...
                                    const fineftp::Filesystem::FileInfo& file_status(*entry.second);
  ...
  void FtpSession::sendNameList(const std::map<std::string, std::shared_ptr<Filesystem::FileInfo>> &directory_content)
  {
    auto data_socket = std::make_shared<asio::ip::tcp::socket>(io_service_);
  ...
  void FtpSession::sendFile(std::shared_ptr<Filesystem::FileIOInterface> file)
  {
    auto data_socket = std::make_shared<asio::ip::tcp::socket>(io_service_);
  ...
  void FtpSession::readDataFromFileAndSend(std::shared_ptr<Filesystem::FileIOInterface> file, std::shared_ptr<asio::ip::tcp::socket> data_socket)
  {
  ...
  void FtpSession::receiveFile(std::shared_ptr<Filesystem::FileIOInterface> file)
  {
    auto data_socket = std::make_shared<asio::ip::tcp::socket>(io_service_);
  ...
  void FtpSession::receiveDataFromSocketAndWriteToFile(std::shared_ptr<Filesystem::FileIOInterface> file, std::shared_ptr<asio::ip::tcp::socket> data_socket)
  {
    std::shared_ptr<std::vector<char>> buffer = std::make_shared<std::vector<char>>(1024 * 1024 * 1);
  ...
  void FtpSession::writeDataToFile(std::shared_ptr<std::vector<char>> data, std::shared_ptr<Filesystem::FileIOInterface> file, std::function<void(void)> fetch_more)
  {
    file_rw_strand_.post([me = shared_from_this(), data, file, fetch_more]
                        {
                          fetch_more();
                          std::string s = "ASSA\n";
                          file->write(s.data(), static_cast<std::streamsize>(s.size()));  //.str("assaASSA\n"); //.write(data->data(), static_cast<std::streamsize>(data->size()));
                        });
  }

  void FtpSession::endDataReceiving(std::shared_ptr<Filesystem::FileIOInterface> file)
  {
  ...
  FtpMessage FtpSession::checkIfPathIsRenamable(const std::string& ftp_path) const
  {
    ...
    if (!ftp_path.empty())
    {
      ...
      auto file_status = Filesystem::createFileInfoObject(toLocalPath(ftp_path));
